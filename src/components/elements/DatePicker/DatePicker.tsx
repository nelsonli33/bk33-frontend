...
export default function DatePicker({
  ...
  const previousMonthName = intl.formatMessage({
    id: `App.DatePicker.months.${monthName(showPreviousMonth)}`,
  });
  const nextMonth = multiMonth
    ? intl.formatMessage({
        id: `App.DatePicker.months.${monthName(showNextToNextMonth)}`,
      })
    : intl.formatMessage({
        id: `App.DatePicker.months.${monthName(showNextMonth)}`,
      });
  ...
  const firstDatePickerAccessibilityLabelPrefix = allowRange
    ? intl.formatMessage({ id: `App.DatePicker.start` })
    : dayAccessibilityLabelPrefix;
  const secondDatePickerAccessibilityLabelPrefix = intl.formatMessage({
    id: `App.DatePicker.end`,
  });
  ...
  const datePickerClassName = classNames("relative rounded bg-white shadow-3");
